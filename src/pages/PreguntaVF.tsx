...
const PreguntaVF = () => {
  ...
  const cargarPreguntas = async (targetPage = page, targetSize = size, targetTextoBusqueda = textoBusqueda) => {
    ...
    try {
      ...
      const resp = targetTextoBusqueda.trim()
        ? await preguntaVFService.search(targetTextoBusqueda.trim(), targetPage, targetSize, filtros)
        : await preguntaVFService.getAll(targetPage, targetSize, filtros);
  ...
  const cambiarEstado = async (p: IPreguntaVF) => {
    ...
    try {
      ...
      cargarPreguntas(page, size); // refrescamos la lista
    } catch (error) {
